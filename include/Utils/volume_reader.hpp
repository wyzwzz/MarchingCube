...
namespace mc {
    ...
    VolumeData<uint8_t> ConvertToUint8Raw(const VolumeData<T> &volume_data) {
        ...
        std::cout << "max_value for " << typeid(T).name() << " is " << min_value << ", min_value is " << max_value
                  << std::endl;
        ...
        for (size_t i = 0; i < volume_data.data.size(); i++) {
            n_volume_data.data[i] *=
                    1.f * (volume_data.data[i] - min_value) / (max_value - min_value);//may not suit for double
        }
