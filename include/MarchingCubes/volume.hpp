...
//            assert(layer.u_slice.size()==0);
            std::copy(volume_data.begin() + slice_size * (z + 1), volume_data.begin() + slice_size * (z + 2),
                      layer.u_slice.data());
...
#pragma omp parallel for
            for (int y = 0; y < volume_y - 1; y++) {
                for (int x = 0; x < volume_x - 1; x++) {
                    ...
                    auto num = std::count_if(values.begin(), values.end(),
                                             std::bind2nd(std::greater<Node>(), Node(isovalue)));
                    ...
                    switch (num) {
                        ...
                        case 4: {
                            assert(code == cube_case_4a || code == cube_case_4b || code == cube_case_4c
                                   || code == cube_case_4d || code == cube_case_4e || code == cube_case_4f ||
                                   code == cube_case_4g);
